<template>
    <div id="buscador">
        <div id="elementos">
            <div id="contenedorElemento">
                <ModalFormularios id="elemento" class="btn m-2"></ModalFormularios>
    ...
    <div id="table" class="table-responsive mt-1">
        <table class="table">
            <thead>
                <tr id="titulosTablaFormulario">
                <th class="estaticoTitulo" scope="col">Id</th>
                <th class="estaticoTitulo2" scope="col">Editar</th>
                <th class="normal" scope="col">Nombre</th>
                <th class="normal" scope="col">Descripcion</th>
                <th class="normal" scope="col">Borrar</th>
                </tr>
            </thead>
            <tbody v-if="cantidadPaginaActual">
                <tr id="letraTabla" v-for="datos in datosState" :key="datos.for_id">
                    <td class="estatico"> {{ datos.for_id }} </td>
                    <td id="botonEditarFila" @click="traerIdEditar(datos.for_id)" data-bs-toggle="modal" data-bs-target="#exampleModalEditFormulario" data-bs-whatever="@mdo"><span class="material-symbols-outlined">edit</span> </td>
                    <td class="normal">{{  datos.for_nombre }}</td>
                    <td class="normal">{{  datos.for_detalle }}</td>
                    <td @click="borrarDato(datos.for_id)" class="normal"><span class="material-symbols-outlined text-danger">DELETE</span></td>
                </tr>         
            </tbody>
        </table>
    </div> 
...
export default {   
    components:{
        ModalFormularios: defineAsyncComponent(() => import('../components/ModalFormularios.vue')),
    },
    setup(){ 
        const { obtenerCliente, datosCliente  } = useAjustes()
        const { ToastInformacion , ToastConfirmacion } = useAlerts()
        const {obtenerDatos , datosState , obtenerCantidadPaginas , datosCantidadPaginasState , irPaginaAnterior , cantidadPaginaActual , obtenerDatosTodos , borrar , obtenerId , datosPorId} = useFormulario()
        ...
        onMounted( async () => {    
            ...
            // document.querySelectorAll("#titulosTablaFormulario").forEach(element => {
        ...
        return  {
            ...
            borrarDato: async( dato )=>{
                ...
                ToastConfirmacion.fire({
                    html: 'El registro se eliminara de forma permanente: <b>' +datosPorId.value[0]['for_nombre'] +'</b>',
                }).then(async(result) => {
                    if (result.isConfirmed) {
                        ToastInformacion.fire({ icon: 'success', html: 'Registro eliminado: <b>' +datosPorId.value[0]['for_nombre'] +'</b>'})
                        await borrar(dato)    
                        obtenerDatos(1)  
                        obtenerCantidadPaginas()
                    }
            ...
            traerIdEditar: async( dato )=>{                    
                await obtenerId(dato)                
                document.getElementById("nombreEditFormulario").value = datosPorId.value[0]['for_nombre']
                document.getElementById("detalleEditFormulario").value = datosPorId.value[0]['for_detalle']
            },
        }
    }

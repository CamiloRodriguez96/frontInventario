    ...
    <div id="buscador">
        <div id="elementos">            
            <div id="contenedorElemento">
                <ModalFormularioDetalles id="elemento" class="btn m-2"></ModalFormularioDetalles>
                <select @change="obtenerDatosForm(busquedaTexto.texto)" class="form-select form-select-sm m-2 " aria-label=".form-select-sm example" id="nombreFormularioFormularioDetalle" v-model="busquedaTexto.texto">
                    <option selected>Selecciona el formulario</option>
                    <option  v-for="formulario in formularios" :key="formulario" :value="formulario.for_nombre" >{{formulario.for_nombre}}</option>
                </select>
    ...
    <div id="table" class="table-responsive mt-1">
        <table class="table">
            <thead>
                ...
                <th class="normal" scope="col">Formulario</th>
                <th class="normal" scope="col">Nombre</th>
                <th class="normal" scope="col">Detalle</th>
                <th class="normal" scope="col">Adjunto</th>
                <th class="normal" scope="col">vigencia</th>
                <th class="normal" scope="col">Borrar</th>
                </tr>
            </thead>
            <tbody v-if="cantidadPaginaActual">
                <tr id="letraTabla" v-for="datos in datosState" :key="datos.fod_id">
                    <td class="estatico"> {{ datos.fod_id }} </td>
                    <td id="botonEditarFila" @click="traerIdEditar(datos.fod_id)" data-bs-toggle="modal" data-bs-target="#exampleModalEditFormularioDetalle" data-bs-whatever="@mdo"><span class="material-symbols-outlined">edit</span> </td>
                    <td class="normal">{{  datos.for_nombre }}</td>
                    <td class="normal">{{  datos.fod_nombre }}</td>
                    <td class="normal">{{  datos.fod_tipo }}</td>
                    <td class="normal">{{  datos.fod_adjunto }}</td>
                    <td class="normal">{{  datos.fod_vigencia }}</td>
                    <td @click="borrarDato(datos.fod_id)" class="normal"><span class="material-symbols-outlined text-danger">DELETE</span></td>
                </tr>         
            </tbody>
        </table>
    </div> 
...
export default {   
    components:{
        ModalFormularioDetalles: defineAsyncComponent(() => import('../components/ModalFormularioDetalles.vue')),
    },
    setup(){ 
        ...
        const {obtenerDatos , datosState , obtenerCantidadPaginas , datosCantidadPaginasState , irPaginaAnterior , cantidadPaginaActual , obtenerDatosForm , borrar , obtenerId , datosPorId} = useFormularioDetalle()
        ...
        onMounted( async () => {    
            ...
            // document.querySelectorAll("#titulosTablaFormularioDetalle").forEach(element => {
        ...
        return  {
            ...
            borrarDato: async( dato )=>{
                ...
                ToastConfirmacion.fire({
                    html: 'El registro se eliminara de forma permanente: <b>' +datosPorId.value[0]['fod_nombre'] +'</b>',
                }).then(async(result) => {
                    if (result.isConfirmed) {
                        ToastInformacion.fire({ icon: 'success', html: 'Registro eliminado: <b>' +datosPorId.value[0]['fod_nombre'] +'</b>'})
                        await borrar(dato)    
                        obtenerDatos(1)  
                        obtenerCantidadPaginas()
                    }
            ...
            traerIdEditar: async( dato )=>{                    
                await obtenerId(dato)                
                document.getElementById("nombreEditFormularioDetalle").value = datosPorId.value[0]['fod_nombre']
                document.getElementById("tipoDetalleEditFormularioDetalle").value = datosPorId.value[0]['fod_tipo']
                document.getElementById("adjuntoEditFormularioDetalle").value = datosPorId.value[0]['fod_adjunto']
                document.getElementById("vigenciaEditFormularioDetalle").value = datosPorId.value[0]['fod_vigencia']
            },
        }
    }
...
#nombreFormularioFormularioDetalle{
    height: 30px;    
    max-width: 200px;
}
